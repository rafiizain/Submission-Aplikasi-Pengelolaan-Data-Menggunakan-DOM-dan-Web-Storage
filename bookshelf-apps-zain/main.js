...
function RenderBookList(bookData) {
  ...
  for (let book of bookData) {
    ...
    containerActionItem.append(moveButton, editButton, deleteButton);
...
function CreateMoveButton(book, eventListener) {
  ...
  moveButton.innerText = isSelesai + " dibaca";
...
function UpdateAnItem(itemElement) {
  ...
  const isComplete = itemElement.childNodes[3].childNodes[0].innerText.length === "Selesai dibaca".length ? false : true;
